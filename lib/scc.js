...
function makeCaptionBlock (buffer, timeStamp, frames) {
    var cap = {
        timeStamp: timeStamp,
    ...
    //if (jsonCaptions[jsonCaptions.length - 1] !== undefined && jsonCaptions[jsonCaptions.length - 1].endTimMicro === undefined) {
    //    console.log("Setting " + JSON.stringify(jsonCaptions[jsonCaptions.length - 1]));
    //    console.log("With end time:" + module.exports.processTimeStamp(timeStamp, frames));
    //    jsonCaptions[jsonCaptions.length - 1].endTimeMicro = module.exports.processTimeStamp(timeStamp, frames);
    //}
...
function rollUp() {
    ...
    makeCaptionBlock(paintBuffer, timeStamp, frameCount);
    paintBuffer = '';
    rollRows = [];
}
...
module.exports = {
    ...
    translateCommand: function translateCommand(word) {
        ...
        if (command === '9420') {
            ...
            //time
            //make caption
            makeCaptionBlock(popBuffer, timeStamp, frameCount);
            popBuffer = '';
           ...
           //     console.log("in 942c");
           //     console.log("Setting " + JSON.stringify(jsonCaptions[jsonCaptions.length - 1]));
           //     console.log("With end time:" + module.exports.processTimeStamp(timeStamp, frameCount));
    ...
    translateCharacters: function translateCharacters (word) {
        if (word.length > 0) {
            ...
            if (mapping.CHARACTERS[chars[0]] === undefined || mapping.CHARACTERS[chars[1]] === undefined) {
                return;
            }
            if (paintOn) {
