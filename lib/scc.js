...
module.exports = {
    ...
    toJSON: function (data) {
        ...
        for (index = 1; index < data.length; index++) {
            if (data[index].trim() !== '' && module.exports.verify(data[index]) === false && data[index].trim() !== 'null' && data[index].trim().length > 0) {
                ...
                if (parsed[3].match(/8080/)) {
                    timeAddCount = parsed[3].match(/8080/g).length * 2;
                    stamp[stamp.length - 1] = parseInt(stamp[stamp.length - 1], 10) + timeAddCount;
                }
                frames = stamp[stamp.length - 1];
                startTimeMicro = module.exports.translateTime(stamp.join(":"));
                captionText = module.exports.translate(parsed[3]);
                endTimeMicro = '';
                if (json.captions.length > 0) {
                    stamp[stamp.length - 1] = json.captions[(json.captions.length - 1)].frames;
                    json.captions[(json.captions.length - 1)].endTimeMicro = module.exports.translateTime(stamp.join(':'));
                    json.captions[(json.captions.length - 1)].durationSeconds = (((json.captions[(json.captions.length - 1)].endTimeMicro - json.captions[(json.captions.length - 1)].startTimeMicro) / 1000) / 1000);
                }
     ...
     */
    translateTime: function (timeStamp, skew) {
        ...
        return (skew ? microSeconds + skew : microSeconds);
    }
};
