...
module.exports = {
    ...
    adjust: function (adjustment, precision, captions, callback) {
        ...
        if (precisionMultipliers[precision] && captions[0].startTimeMicro !== undefined) {
            ...
            //quick check to see if it will zero out the 1st 2nd or 3rd caption
            if ((captions[0].startTimeMicro + adjuster) <= 0 || (captions[1].startTimeMicro + adjuster)  <= 0 || (captions[2].startTimeMicro + adjuster) <= 0) {
    ...
    /**
     * Automatically adjust timecodes to prevent stacking
     * @function
     * @param {array} captions - json array of captions to perform adjustment on created by toJSON
     * @public
     */
    autoAdjust: function (captions, callback) {
        var capIdx = 0;

        for (capIdx = 0; capIdx < captions.length; capIdx++) {
            if (capIdx > 0) {
                if (captions[capIdx].startTimeMicro < captions[(capIdx - 1)].endTimeMicro) {
                    captions[capIdx].startTimeMicro = captions[(capIdx - 1)].endTimeMicro;
                }
            }
        }
        callback(captions);
